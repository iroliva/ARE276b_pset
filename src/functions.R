...
my_ivreg <- function(y, controls, endog_var, instrument, data) {
  formula <- as.formula(paste(y, "~", paste(controls, collapse = " + "), "|", 
    paste(endog_var, collapse = " + "), "|", paste(instrument, collapse = " + ")))
...
make_balance_table <- function(var, controls, database, column_name, table_title, table_label, table_out) {
  results <- list()
...
make_figure <- function(data, x_var, y_var, y_axis_limits, y_label, x_label, file_name) {
  my_figure <- ggplot(data %>% filter(mtspgm74 >= 25 & mtspgm74 <= 125),
        aes_string(x = x_var, y = y_var, color = "factor(tsp75)")) +
